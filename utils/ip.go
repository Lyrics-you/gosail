...
func ParseIp(ip string) ([]string, error) {
	...
	if strings.Contains(ip, "/") {
		...
		if strings.Contains(ip, "/32") {
			...
			availableIPs, err := GetAvailableIP(ip)
			if err != nil {
...
func GetAvailableIP(ipAndMask string) ([]string, error) {
	var availableIPs []string
...
func IPMaskCIDRToString(one string) (string, error) {
	...
	return strings.Join(maskstring, "."), nil
}
